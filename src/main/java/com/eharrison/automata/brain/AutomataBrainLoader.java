 ...
 *
 * Note: This creates instances of the brains as services, so there is duplication of the instances that is not used.
 * // TODO refactor to have the brain service create new instances of the brain.
 */
public class AutomataBrainLoader {
    ...
    public Map<String, Class<? extends AutomataBrain>> loadBrains() throws IOException {
        val brains = new HashMap<String, Class<? extends AutomataBrain>>();
            ...
            for (val jarFile : jarFiles) {
                try (val classLoader = new URLClassLoader(
                    ...
                    for (val brain : serviceLoader) {
                        brains.put(brain.name(), brain.getClass());
                    }
                }
            }
        }
