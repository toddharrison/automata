...
public class RPSGame implements Game<RPSConfig, RPSState, RPSView, RPSAction, RPSResult, RPSBot> {
    @Override
    ...
    @Override
    public RPSResult run(final RPSConfig config, final List<RPSBot> bots) {
        ...
        while (!isGameOver(state)) {
            ...
            val score1 = isWin(action1, action2) ? state.bot1Score() + 1 : state.bot1Score();
            val score2 = isWin(action2, action1) ? state.bot2Score() + 1 : state.bot2Score();

            state = state.next(score1, score2, action1, action2);
        }

        return new RPSResult(state.round(), Map.of(state.bot1(), state.bot1Score(), state.bot2(), state.bot2Score()));
    }
